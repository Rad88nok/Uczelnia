...
public class PerceptronLearningAlgorithm  {
    ...
    public void treining(double alpha){
        ...
        myPerceptron= new MyPerceptron(Training.get(0).getValues().size(),alpha);//ilość x i alpha
        for(MySet mySet:Training){
    ...
    public void test(){
        ...
        for(MySet mySet:Test){
            int y=myPerceptron.enumeration(mySet);
            if(answerMap.get(mySet.getName())==y&& answerMap.get(names[0])==y ){
                correctAnswerFC++;
            }
            if(answerMap.get(mySet.getName())==y&& answerMap.get(names[1])==y ){
                correctAnswerSC++;
            }
        }
    ...
    public void newTest() {
        ...
        int y = myPerceptron.enumeration(mySetNewTest);
        for (String key : answerMap.keySet()){
